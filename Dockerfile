...
ARG S6_OVERLAY_REF="f1fecfd867a13723588aa3ed6fe2ce480860dda3"

# https://bearssl.org/gitweb/?p=BearSSL;a=commit;h=7bea48e5e850ab4cafbe68d3765cdaba13a86d6f
ARG BEARSSL_VERSION="0.6"
ARG BEARSSL_REF="7bea48e5e850ab4cafbe68d3765cdaba13a86d6f"

ARG SKALIBS_VERSION="2.15.0.0"
ARG EXECLINE_VERSION="2.9.9.0"

ARG S6_VERSION="2.15.0.0"
ARG S6_RC_VERSION="0.6.1.0"
ARG S6_LINUX_INIT_VERSION="1.2.0.1"
ARG S6_PORTABLE_UTILS_VERSION="2.3.1.2"
ARG S6_LINUX_UTILS_VERSION="2.6.4.1"
ARG S6_DNS_VERSION="2.4.1.2"
ARG S6_NETWORKING_VERSION="2.8.0.0"
ARG S6_OVERLAY_HELPERS_VERSION="0.1.2.2"
...
RUN <<EOT
  set -ex
  DESTDIR=/out ./configure --host=$(xx-clang --print-target-triple) --enable-slashpackage --enable-static-libc --disable-shared --with-default-path=/command:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin --with-sysdep-devurandom=yes --with-sysdep-grndinsecure=no --with-sysdep-posixspawnearlyreturn=no --with-sysdep-procselfexe=/proc/self/exe
  make -j$(nproc)
